...
class Protocol:
    ...
    # 
    def handle_command_0x1001(self, data_bytes):
        """
            开始工作 / 下一个的标志("start" / "next")
            :param data_bytes: 数据段 (bytes)
            :return: 解析后的数据
        """
        single = struct.unpack('<5s',data_bytes)
        return single
    

    def build_command_0x3001(self, class_id, center_x, center_y):
        """
            nagisa队的yolo数据构造包
            :param data_bytes: 数据段 (bytes)
            :return: 解析后的数据
        """
        command = 0x3001
        data_bytes = struct.pack('<BBB', center_x, center_y, class_id) 
        return self.construct_frame(command, data_bytes)
